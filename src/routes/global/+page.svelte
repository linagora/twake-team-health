<script lang="ts">
	...
	import { orgTrend, avgOver, type OrgMonth } from '$lib/charts';
	import { fmtMonth, fmtNum } from '$lib/utils';
	import { AlertCircle, Loader2, FileDown, ArrowUp, ArrowDown, Minus } from '@lucide/svelte';
	...
	// it is one period, everything from it on is the other.
	let splitMonth = $state<string>('');
	$effect(() => {
		if (!splitMonth && months.length >= 4) splitMonth = months[Math.floor(months.length / 2)];
	});
	...
	{:else if trend.length}
		...
		<section>
			<div class="mb-5">
				...
				<h3 class="font-display text-[1.6rem] leading-none tracking-tight">How the org is shipping</h3>
			</div>
